...
<p align="center"><b>永続インデックスと差分同期対応の grep ライク全文検索ツール</b></p>
...
- **永続インデックス + 自動 sync** — 一度インデックスを作れば即時検索。検索前に mtime ベースの差分 sync を自動実行
- **行モード・ファイルモード** — 行モードは grep 風結果、ファイルモードは frontmatter 対応のドキュメント検索
- **多言語対応** — 日本語形態素解析（IPADIC）と bigram フォールバックによる幅広いカバレッジ
